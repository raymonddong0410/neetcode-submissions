class Solution:
    def productExceptSelf(self, nums: List[int]) -> List[int]:
        ...
        pre = 1
        for i in range(x):
            # what this does is build the prefix in place and if nums[0] is 0 then everything ends up in res as 0 
            # same idea as above but not using an array, instead a constant
            res[i] = pre
            pre *= nums[i]
        
        post = 1
        for i in range(x-1, -1, -1):
            # at this point you know last elem will just be what the pre prod will be cause there's nothing after
            # other spots multiple the post by pre prod already calced and held in the res
            res[i] *= post
            post *= nums[i]
